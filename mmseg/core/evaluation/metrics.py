...
def eval_metrics(results,
    ...
    glosses = 0.0
    nglosses = 0.0
    miou_building = np.asarray([0.0, 0.0])
    ...
    for i in range(len(results)):
        ...
        tree_cls = cls_target==5
        building_cls = cls_target==5
        building_cls = building_cls.astype(np.uint8)
        out = out.astype(np.uint8) / 255
        area_intersect, area_union, area_pred_label, area_label = intersect_and_union(out, building_cls, 2, -1)
        #IoU = area_intersect / area_union
        miou_building += np.asarray([area_intersect[1],area_union[1]])
        #pdb.set_trace()'''
        #print(out)
        #cv2.imwrite('/home/xshadow/Swin-Transformer-Semantic-Segmentation/response_new/tree_{}.png'.format(str(i)), tree_cls.astype(np.uint8)*255)
        #cv2.imwrite('/home/xshadow/Swin-Transformer-Semantic-Segmentation/response_new/building_{}.png'.format(str(i)), building_cls.astype(np.uint8)*255)
        #cv2.imwrite('/home/xshadow/Swin-Transformer-Semantic-Segmentation/response_new/bwbuilding_' + str(i) + '.png', out.astype(np.uint8))
        #print('semantic map saved.')
        
        '''if i %10 ==0:
            out = out.astype(np.float16)
            np.save('/home/xshadow/Swin-Transformer-Semantic-Segmentation/cross_height_out/height_out_{}.npy'.format(str(i)), out)
            print('saved: {}'.format(i))
            np.save('/home/xshadow/Swin-Transformer-Semantic-Segmentation/cross_height_out/target_{}.npy'.format(str(i)), target)
        #'''
    ...
    miou_building = miou_building[0] / miou_building[1]
    print('')
    print('---------------------------------------------------')
