...
class UPerHead(BaseDecodeHead):
    ...
    def forward(self, inputs):
        ...
        '''
        print(self.height_out.conv2d.weight.min(),self.height_out.conv2d.weight.max())
        L1hist,_ = np.histogram(self.height_out.conv2d.weight.cpu().detach().numpy()[0,:128,...], bins=50,range=[-0.12,0.2])
        L2hist,_ = np.histogram(self.height_out.conv2d.weight.cpu().detach().numpy()[0,128:256,...], bins=50,range=[-0.12,0.2])
        L3hist,_ = np.histogram(self.height_out.conv2d.weight.cpu().detach().numpy()[0,256:384,...], bins=50,range=[-0.12,0.2])
        L4hist,_ = np.histogram(self.height_out.conv2d.weight.cpu().detach().numpy()[0,384:,...], bins=50,range=[-0.12,0.2])
        for i in L1hist:
            print(i)
        pdb.set_trace()
        #print(output.shape) #N, 512, H, W
        '''
