#include <vector>
#include "cuda/ps_deform_im2col_cuda.cuh"
                    ...
                    const int im2col_step)
{
    ...
    AT_ASSERTM(scale_coef.type().is_cuda(), "scale_coef must be a CUDA tensor");
    ...
    const int per_scale_coef_size = scale_coef.size(1) * scale_coef.size(2) * scale_coef.size(3);
    ...
    for (int n = 0; n < batch/im2col_step_; ++n)
    {
        ...
        AT_DISPATCH_FLOATING_TYPES(input.type(), "deform_conv_forward_cuda", ([&] {
            deformable_im2col_cuda(at::cuda::getCurrentCUDAStream(),
                                             ...
                                             scale_coef.data<scalar_t>() + n * im2col_step_ * per_scale_coef_size,
                                             ...
                                             const int im2col_step)
{
    ...
    AT_ASSERTM(bias.type().is_cuda(), "bias must be a CUDA tensor");
    AT_ASSERTM(offset.type().is_cuda(), "offset must be a CUDA tensor");
    AT_ASSERTM(scale_c.type().is_cuda(), "offset must be a CUDA tensor");
    ...
    auto grad_scale_c = at::zeros_like(scale_c);
    ...
    const int per_input_size = channels * height * width;
    const int per_offset_size = offset.size(1) * offset.size(2) * offset.size(3);
    const int per_scale_c_size = scale_c.size(1) * scale_c.size(2) * scale_c.size(3);
    ...
    for (int n = 0; n < batch/im2col_step_; ++n)
    {
        ...
        AT_DISPATCH_FLOATING_TYPES(input.type(), "deform_conv_backward_cuda", ([&] {
            deformable_col2im_coord_cuda(at::cuda::getCurrentCUDAStream(),
                                                   ...
                                                   input.data<scalar_t>() + n * im2col_step_ * per_input_size,
                                                   offset.data<scalar_t>() + n * im2col_step_ * per_offset_size,
                                                   scale_c.data<scalar_t>() + n * im2col_step_ * per_scale_c_size,
                                                   batch_n, channels, height, width,
                                                   height_out, width_out, kernel_h, kernel_w,
                                                   pad_h, pad_w, stride_h, stride_w,
                                                   dilation_h, dilation_w, deformable_group,
                                                   grad_offset.data<scalar_t>() + n * im2col_step_ * per_offset_size,
                                                   grad_scale_c.data<scalar_t>() + n * im2col_step_ * per_scale_c_size);
