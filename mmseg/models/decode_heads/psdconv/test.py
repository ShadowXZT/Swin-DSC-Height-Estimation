...
def check_psdconv_zero_offset():
    ...
    conv_scale_c = nn.Conv2d(inC, deformable_groups * 1 * kH * kW,
                          kernel_size=(kH, kW),
                          stride=(1, 1),
                          padding=(1, 1),
                          bias=True).cuda()

    dcn = PSDeformConv(inC, outC, (kH, kW),
    ...
    conv_scale_c.weight.data.zero_()
    conv_scale_c.bias.data.zero_()

    input = torch.randn(N, inC, inH, inW).cuda()
    offset = conv_offset(input)
    scale_c = conv_scale_c(input)
    scale_c = torch.sigmoid(scale_c)
    scale_c *= 2  #Make the init scale to be 1
    output_d = dcn(input, offset, scale_c)
    output_p = pcn(input)
    d = (output_d - output_p).abs().max()
    if d < 1e-5:
        print('psdconv zero offset passed with {}'.format(d))
    else:
        print('psdconv zero offset failed with {}'.format(d))
        # print(output_p)
        # print(output_d)
        print((output_d - output_p).abs())


def check_psdconv_zero_offset_identify():
    conv_offset = nn.Conv2d(inC, deformable_groups * 2 * kH * kW,
                            kernel_size=(kH, kW),
                            stride=(1, 1),
                            padding=(1, 1),
    ...
    input = torch.randn(N, inC, inH, inW).cuda()
    offset = conv_scale_c(input)
    scale_c = conv_scale_c(input)
    scale_c = torch.sigmoid(scale_c)
    output = dcn(input, offset, scale_c)
    output *= 2
    d = (input - output).abs().max()
    if d < 1e-10:
        print('psdconv zero offset identify passed with {}'.format(d))
    else:
        print('psdconv zero offset identify failed with {}'.format(d))
        # print(input)
        # print(output)
        print((input - output).abs())

def check_psdconv_im2col_step_forward():
    conv_offset = nn.Conv2d(inC, deformable_groups * 2 * kH * kW,
                            kernel_size=(kH, kW),
                            stride=(1, 1),
                            padding=(1, 1),
    ...
    dcn1 = PSDeformConv(inC, outC, (kH, kW), 
        stride=1, padding=1, dilation=1, 
        groups=groups, 
        deformable_groups=deformable_groups,
        im2col_step=1).cuda()

    dcn2 = PSDeformConv(inC, outC, (kH, kW), 
        stride=1, padding=1, dilation=1, 
        groups=groups, 
        deformable_groups=deformable_groups,
        im2col_step=2).cuda()
    ...
    dcn1.bias = dcn2.bias
    output1 = dcn1(input, offset, scale_c)
    output2 = dcn2(input, offset, scale_c)

    d = (output1 - output2).abs().max()
    if d < 1e-10:
        print('psdconv im2col_step forward passed with {}'.format(d))
    else:
        print('psdconv im2col_step forward failed with {}'.format(d))
        print(output1)
        print(output2)
        print((output1 - output2).abs())

def check_psdconv_im2col_step_backward():
    stride = 1
    padding = 1
    groups = 2
    dilation = 1
    ...
    scale_c = torch.sigmoid(torch.randn(N, deformable_groups * 1 * kW * kH, inH, inW).cuda())
    scale_c.requires_grad = True
    ...
    output1 = _PSDeformConv(input, offset, scale_c, weight, bias, stride, padding, dilation, groups, deformable_groups, 2)
    ...
    scale_c_grad = scale_c.grad.clone()
    weight_grad = weight.grad.clone()
    bias_grad = bias.grad.clone()
    output2 = _PSDeformConv(input, offset, scale_c, weight, bias, stride, padding, dilation, groups, deformable_groups, 1)
    ...
    scale_c_grad_err = (scale_c.grad - 2 * scale_c_grad).abs().max()
    weight_grad_err = (weight.grad - 2 * weight_grad).abs().max()
    bias_grad_err = (bias.grad - 2 * bias_grad).abs().max()
    grad_err = input_grad_err + offset_grad_err + scale_c_grad_err + weight_grad_err + bias_grad_err
    if grad_err < 1e-7:
        print("psdconv im2col_step backward passed with {}".format(grad_err))
    else:
        print("psdconv im2col_step backward failed with {}".format(grad_err))
...
def check_gradient_psdconv():
    ...
    scale_c = torch.rand(N, deformable_groups * 1, inH, inW).cuda()
    # mask.data.zero_()
    scale_c.requires_grad = True
    scale = scale_c.repeat(1,kW * kH,1,1)
    scale_c = torch.sigmoid(scale_c)
    ...
    print('check_gradient_psdconv: ',
          gradcheck(_PSDeformConv, (input, offset, scale, weight, bias,
...
def example_psdconv():
    ...
    dcn = PSDeformConvPack(64, 128, kernel_size=(3, 3), stride=1,
...
if __name__ == '__main__':
    ...
    print('checking')
    check_psdconv_im2col_step_forward()
    check_psdconv_im2col_step_backward()
    #check_pooling_zero_offset()
    # zero offset check
    #if inC == outC:
    #    check_psdconv_zero_offset()
        #check_psdconv_zero_offset_identify()

    check_gradient_conv()
    check_gradient_psdconv()
